...
if(mysqli_num_rows($reviewed)>0){
    while($row = mysqli_fetch_array($reviewed)){
      ...
      //other documents:
      $Odocument = $row['File_Location'].$row['Other_Document'];
      $ext_Od = pathinfo($qualification, PATHINFO_EXTENSION);
      ...
      $allowedfileExtensions2 = array('ppt', 'pptx', 'xls', 'xlsx', 'doc', 'docx', 'pdf');
        ...
        if($zip->open($zipname, ZipArchive::CREATE)===True){
          ...
          //adding qualification:
           ...
           //adding other document:
            for ($i=0 ; $i < sizeof($allowedfileExtensions2); $i++){
            if($ext_Od == $allowedfileExtensions2[$i]){
            $zip->addFile($Odocument , $row['Full_Name'].'_supportive_document.'.$allowedfileExtensions2[$i]);
            }
          }
        }
    }
