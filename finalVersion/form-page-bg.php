...
// Rename Other Document File
$Other_DocumentFile = $_FILES['Other_Document']['name'];
$Other_DocumentFileCmps = explode(".", $Other_DocumentFile);
$Other_DocumentFileExtension = strtolower(end($Other_DocumentFileCmps));
$Other_Document_Full_Name_New = implode("", array_map("trim", explode(" ", $Full_Name)));
$Other_Document = $Other_Document_Full_Name_New . ' Other Document.' . $Other_DocumentFileExtension;
...
$sql = "INSERT INTO applies (jobName , Full_Name, Email_Address, IC, Phone_No, DOB, Gender, Nationality, Id_Passport, 
Work_Experience, Level_Of_Education, Qualifications, Other_Document, Other_Qualification, 
CV, Cover_Letter,
Level_Of_Education_Index , Status , File_Location) 
VALUES ('$jobName' ,'$Full_Name', '$Email_Address', '$IC', '$Phone_No', '$DOB', '$Gender', '$Nationality', '$Id_Passport', 
'$Work_Experience', '$Level_Of_Education', '$Qualifications', '$Other_Document', '$Other_Qualification', 
'$CV', '$Cover_Letter',
'$Level_Of_Education_Index' , '$Status' , '$File_Location')";
...
if (isset($_POST['Submit']) && $_POST['Submit'] == 'Submit Form') {
  ...
  if(isset($_FILES['Other_Document'])){
    $Other_Document_size = $_FILES['Other_Document']['size'];
    $Other_Document_tmp = $_FILES['Other_Document']['tmp_name'];

    if (in_array($Other_DocumentFileExtension, $allowedfileExtensions2)) { 
      $dest_path = $uploadLocation . $Other_Document; // change $Cover_Letter

      if(move_uploaded_file($Other_Document_tmp, $dest_path)) { 
        $message = 'File uploaded successfully.';
      } else {
        $message = 'An error occurred while uploading the file to the destination directory. Ensure that the web server has access to write in the path directory.';
      }
    ...
    $message = 'Error occurred while uploading the file.<br>';
    $message .= 'Error:' . $_FILES['Other_Document']['error']; 
  }
